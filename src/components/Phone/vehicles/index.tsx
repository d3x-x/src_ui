...
export default class Vehicles extends Component<{}, State> {
	...
	async getVehiclePosition() {
		...
		try {
			...
			showNotification( 'info', 'Розташування ТЗ вiдмiчено на картi' );
		} catch ( err: any ) {
			showNotification( 'error', 'Спочатку замовте це ТЗ' );
		}
	}
	...
	async spawnVehicle() {
		...
		try {
			...
			showNotification( 'info', 'Ваше ТЗ скоро буде доставлено' );
		} catch ( err: any ) {
	...
	async despawnVehicle() {
		...
		try {
			...
			showNotification( 'info', 'Чекайте на евакуацiю найближчим часом.' );
		} catch ( err: any ) {
			if ( err.msg ) showNotification( 'error', err.msg );
		}
	}
