	...
	async onControlClick( type: string ) {
		...
		if ( type !== 'call' ) return;
			try {
				...
				showNotification('error', 'Абонент тимчасово недоступний');
			}
		}
		...
		render() {
			...
			return (
				<div className="contacts_contact">
					<Navigation
						close={{ title: 'Контакти', onClick: close }}
						action={{ title: 'Редагувати', onClick: showEditor }}
					/>
		
					<div className="contacts_contact-header">
						<div className="avatar">{`${contact.firstName.charAt(0)}${contact.lastName.charAt(0)}`}</div>
