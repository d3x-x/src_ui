		...
		return (
			<div className="trading trading--business">
				<div className="trading_top">
					...
					<div className="trading_info">
						<div className="trading_info-items">
							...
							Держ. цiна: <b>{prettify.price(govPrice)}</b>
							</p>
							<p className="trading_info-item">
								Дохiд: <b>{prettify.price(income)}</b>
							</p>
						</div>
					</div>
				</div>
				...
				<div className="trading_footer">
					<OutlineButton onClick={this.refuse}>Закрити</OutlineButton>

					{seller ? (
						<GradientButton onClick={this.buy}>Придбати</GradientButton>
					) : (
						<GradientButton onClick={this.sell.bind(this)}>Запропонувати</GradientButton>
					)}
				</div>
			</div>
		);
