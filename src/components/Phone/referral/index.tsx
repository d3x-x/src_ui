...
export default class Referral extends Component<{}, State> {
	...
	async useCode( code: string ) {
		try {
			...
			showNotification( 'success', 'Промокод успiшно активовано' );
		} catch ( err: any ) {
		...
		return (
			<div className="referral">
				<Group className="referral_info">
					...
					<Button current={referrals.confirmed.toString()}>Отримали бонус</Button>
				</Group>

				<Description>{`Пiсля досягнення ${ confirmLevel }го рiвня гравця, вiн отримає ${ bonus }$, а ви ${ income }$.`}</Description>
