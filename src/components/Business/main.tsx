		...
		return (
			<div className="business_main">
				...
				<div className="business_info">
					<BusinessInfoItem title="Власник">{owner || 'Вiдсутнє'}</BusinessInfoItem>
					<BusinessInfoItem title="Прибуток за добу">
						{prettify.price(income)}
					</BusinessInfoItem>
					<BusinessInfoItem title="Вартiсть на день">
						{prettify.price(tax)}
					</BusinessInfoItem>
					<BusinessInfoItem title={isOwner ? 'Держ. продаж' : 'Цiна'}>
						{prettify.price(price)}
					</BusinessInfoItem>

					{isOwner && (
						<div className="business_info-owned">
							<BusinessInfoItem title="Оплачено днiв">{paid.toString()}</BusinessInfoItem>
							<BusinessInfoItem title="До прибутку">{`${hour
								.toString()
